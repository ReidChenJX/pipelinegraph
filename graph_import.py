...
class ImportGraph(CreateGraph):
    ...
    def file_to_node(self):
        ...
        out_relation[':END_ID(road)'] = out_relation[':END_ID(road)'].apply(
            lambda x: str(x).replace('终点道路：', '').strip())
        ...
        be_relation_man.rename({'name:ID(manhole)': ':START_ID(manhole)', 'road_name': ':END_ID(road)'}, axis=1,
                               inplace=True)
        be_relation_man[':END_ID(road)'] = be_relation_man[':END_ID(road)'].apply(
            lambda x: str(x).replace('所属道路：', '').strip())
        ...
        in_relation_man.rename({'name:ID(manhole)': ':START_ID(manhole)', 'in_roadname': ':END_ID(road)'}, axis=1,
                               inplace=True)
        in_relation_man[':END_ID(road)'] = in_relation_man[':END_ID(road)'].apply(
            lambda x: str(x).replace('起始道路：', '').strip())
        ...
        out_relation_man.rename({'name:ID(manhole)': ':START_ID(manhole)', 'out_roadname': ':END_ID(road)'}, axis=1,
                                inplace=True)
        out_relation_man[':END_ID(road)'] = out_relation_man[':END_ID(road)'].apply(
            lambda x: str(x).replace('终点道路：', '').strip())
